  ...
  return <main className="journal-site jr-page">
    ...
    <div className="jr-layout"><aside className="jr-sidebar">{entries.length || editor ? <><p className="jl-eyebrow">IN THIS JOURNEY</p><h2>沿途的故事</h2>{chapters}</> : null}<div className="jr-side-actions">{hasMap || editor ? <button type="button" onClick={() => setPanel("map")}>⌖ 旅程地圖</button> : null}<a href="#album" onClick={event => jump(event, "album")}>▧ 旅途相簿</a>{videos.length || filmPhotos.length ? <button type="button" onClick={() => setPanel("film")}>▷ 旅程短片</button> : null}</div><ShareActions title={trip.title} description={trip.summary} path={`/trips/${trip.slug}?id=${encodeURIComponent(trip.id)}`} /></aside>
      <div className="jr-story"><p className="jr-opening">把腳步放慢，回到旅途裡。</p>
        ...
        {photos.length || editor ? <section id="album" className="jr-album">{editor ? <button className="jr-photo-edit-static" type="button" onClick={()=>editor.onEdit({kind:"album"})}>編輯相簿・上傳照片</button> : null}<ReaderAlbum photos={photos} /></section> : null}
