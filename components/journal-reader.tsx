      ...
      <div className="jr-story"><p className="jr-opening">把腳步放慢，回到旅途裡。</p>
        {entries.map((entry, index) => {
          ...
          return <article id={`story-${entry.id}`} data-reader-chapter className="jr-chapter" key={entry.id} data-music-zone={`${entry.title} ${entry.body}`}>
            ...
            {photo ? <figure className="jr-story-photo"><div className="jr-edit-photo"><ReaderPhoto key={photo.id} photo={photo} />{editor ? <button className="jr-photo-edit" type="button" onClick={()=>editor.onEdit({kind:"photo",id:photo.id,entryId:entry.id})}>修改這張照片</button> : null}</div>{photo.caption ? <figcaption>{photo.caption}</figcaption> : null}</figure> : null}
