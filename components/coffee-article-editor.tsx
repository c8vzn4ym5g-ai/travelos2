...
export function CoffeeArticleEditor({shop,onChange,onSave,onAdvanced,busy,message,embedded=false,readingMode=false}:{shop:CoffeeShop;onChange:(shop:CoffeeShop)=>void;onSave:()=>void;onAdvanced:()=>void;busy:boolean;message:string;embedded?:boolean;readingMode?:boolean}) {
  const [localReading,setReading]=useState(false);
  const reading=embedded?readingMode:localReading;
  const Container=embedded?"section":"main";
  const photos=shop.photos.filter(photo=>photo.storageKey.startsWith("/")||photo.storageKey.startsWith("http"));
  ...
  return <Container className="bg-[#f7f4ed] pb-10 text-[#283d33]">
    ...
    <article className="mx-auto max-w-3xl px-5 py-8 sm:px-8 sm:py-12">
      ...
      {photos.map((photo,index)=><section key={photo.id} className="mb-10">
        <figure className="overflow-hidden rounded-2xl"><img alt={photo.caption||photo.originalFilename} src={photo.storageKey} className="max-h-[80vh] w-full object-contain" loading={embedded?'lazy':index===0?'eager':'lazy'}/>{reading?<figcaption className="py-3 text-sm leading-7 text-stone-500">{photo.caption}</figcaption>:<textarea aria-label={`照片 ${index+1} 說明`} className={`${field} mt-3 text-sm leading-7`} rows={2} value={photo.caption??''} onChange={e=>onChange({...shop,photos:shop.photos.map(p=>p.id===photo.id?{...p,caption:e.target.value}:p)})}/>}</figure>
      ...
      {!photos.length?(reading?<p className="whitespace-pre-wrap leading-9">{shop.lifeNote}</p>:<textarea aria-label="文章內文" className={field} rows={10} value={shop.lifeNote} onChange={e=>onChange({...shop,lifeNote:e.target.value})}/>):null}
