...
/**
 * Travelpayouts Drive (emrldtp.cc) patches <a href> / click handling and stays
 * in the persistent root layout after client navigations. Load it only on the
 * booking desk so trip articles and hubs keep working Next.js Links.
 */
export function shouldLoadTravelpayoutsDrive(pathname: string | null | undefined) {
  ...
  return isBookingDesk(normalized);
}
