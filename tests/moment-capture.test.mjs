...
test("capture does not create trips and photos append to a moment", async () => {
  ...
  assert.match(momentsApi, /getMomentById\(momentId\)/);
  assert.match(momentsApi, /searchParams.get\("id"\)/);
  assert.match(upload, /export async function fetchCaptureMoment/);
  assert.match(upload, /readJsonWithTimeout/);
  ...
  assert.doesNotMatch(photosApi, /findWarehousePhotoByContentHash/);
  assert.doesNotMatch(photosApi, /hashPhotoBytes/);
  assert.doesNotMatch(photosApi, /duplicate: true/);
...
test("background upload starts on add and Save does not wait on originals", async () => {
  ...
  assert.match(retryUi, /name="refresh"/);
  assert.match(retryUi, /aria-label="再送"/);
  assert.doesNotMatch(retryUi, />\s*再傳\s*</);
  assert.doesNotMatch(retryUi, />\s*再送\s*</);
  assert.doesNotMatch(retryUi, /全部再送/);
  assert.doesNotMatch(retryUi, /再送一次/);
  ...
  assert.doesNotMatch(uploadFn, /uploaded\.duplicate/);
  ...
  assert.doesNotMatch(displayPost, /findWarehousePhotoByContentHash/);
  assert.doesNotMatch(displayPost, /hashPhotoBytes/);
...
test("write uses a found set for photos and does not fill the writing area", async () => {
  ...
  assert.match(write, /cleanWriteWarehouseDuplicates/);
  assert.doesNotMatch(capture, /\/api\/moments\/dedupe/);
  assert.doesNotMatch(photosApi, /hashPhotoBytes/);
